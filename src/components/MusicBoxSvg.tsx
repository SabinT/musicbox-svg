...
export default class MusicBoxSvg extends React.Component<
  IMusicBoxSvgProps,
  {}
> {
  ...
  public render() {
    ...
    try {
      ...
      const noteGap: number =
        mbProfile.contentWidthMm / (mbProfile.supportedNotes.length - 1);
      const noteOffsetY: number =
        (mbProfile.paperWidthMm - mbProfile.contentWidthMm) / 2;
      ...
      pages.forEach((p) => {
        totalPaperLength +=
          (p.endTimeInSeconds - p.startTimeInSeconds) *
          mbProfile.millimetersPerSecond;
      });
  ...
  private generateSvgPages(
    pages: IMidiEventPage[],
    noteIndices: Map<MidiNote, number>,
    noteGap: number,
    noteOffsetY: number
  ): JSX.Element[] {
    let outPages: JSX.Element[] = [];
    let svgChunks: JSX.Element[] = [];
    ...
    for (const page of pages) {
      ...
      const drawLeadingBorder =
        (formatOptions && !formatOptions.omitPageBoundaries) ||
        page.pageNum === 0;
      const drawTrailingBorder =
        (formatOptions && !formatOptions.omitPageBoundaries) ||
        page.pageNum === pages.length - 1; // last page
        ...
        <>
          <g>
            {formatOptions && formatOptions.renderBorder && (
              <>
                {this.renderSvgLine(
                  page.pageNum === 0 && !this.props.formatting.loopMode
                    ? this.calculateStartSkew()
                    : 0,
                  0,
                  xLen,
                  0
                )}
                {this.renderSvgLine(0, yLen, xLen, yLen)}
                {drawLeadingBorder &&
                  this.renderLeadingBorder(
                    yLen,
                    /* isFirstPage: */ page.pageNum === 0
                  )}
                {drawTrailingBorder &&
                  this.renderTrailingBorder(
                    xLen,
                    yLen,
                    /* isLastPage: */ page.pageNum === pages.length - 1
                  )}
              </>
            )}
          </g>
    ...
    const chunksPerSheet = Math.floor(
      this.props.formatting.pageHeightMm / chunkHeight
    );
    ...
    for (let i = 0; i < numPages; i++) {
      ...
      const w = this.props.formatting.pageWidthMm;
      const h = chunkHeight * chunksPerSheet;
  ...
  private renderLeadingBorder(height: number, isFirstPage: boolean) {
    if (isFirstPage && !this.props.formatting.loopMode) {
      ...
      return this.renderSvgLine(skewMm, 0, 0, height);
    } else {
      if (this.props.formatting.jigsawJoiners) {
        // Render jigsaw joiner so that it can join with previous page
        // (also enabled in loop mode for first page)
        return this.renderJigsawJoiner(/* xStart: */ 0, height);
      } else {
        // Just a boring straight line
        return this.renderSvgLine(0, 0, 0, height);
      }
    }
  }
  ...
  private renderTrailingBorder(
    xLen: number,
    yLen: number,
    isLastPage: boolean
  ) {
    if (
      !this.props.formatting.jigsawJoiners ||
      (isLastPage && !this.props.formatting.loopMode)
    ) {
      // Just a boring straight line
      return this.renderSvgLine(xLen, 0, xLen, yLen);
    } else {
  ...
  private renderSvgLine(x1: number, y1: number, x2: number, y2: number) {
    return (
      <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="black" strokeWidth={0.25} />
    );
  }
  ...
  private renderJigsawJoiner(xStart: number, height: number) {
    ...
    const gap =
      0.5 *
      (profile.paperWidthMm - profile.contentWidthMm - profile.holeDiameterMm);
    ...
    for (let i = 0; i < points.length - 1; i++) {
      const p = points[i];
      const q = points[i + 1];
      lines.push(this.renderSvgLine(p.x, p.y, q.x, q.y));
    }
  ...
  private paginateEvents(): IMidiEventPage[] {
    ...
    if (unsupportedEvents.length > 0) {
      this.errors.push(
        `Unsupported notes found: ${unsupportedEvents.length} total. Inspect MIDI file or music box profile`
      );
    }
    ...
    let currentPage: IMidiEventPage = {
      ...
      startTimeInSeconds:
        -(formatOptions.startPaddingMm + musicBoxProfile.holeDiameterMm * 0.5) /
    ...
    const secondsPerHoleRadius =
      (musicBoxProfile.holeDiameterMm * 0.5) /
      musicBoxProfile.millimetersPerSecond;
    const minNoteGapSeconds =
      musicBoxProfile.minNoteGapMm / musicBoxProfile.millimetersPerSecond;
    ...
    while (currentNoteIndex < supportedEvents.length) {
      ...
      const maxPageEndTime =
        currentPage.startTimeInSeconds + pageLengthInSeconds;

      // Coalesce/skip notes that are too close
      if (
        previousNoteEvent &&
        previousNoteEvent.note === noteEvent.note &&
        noteEvent.absTimeSeconds <=
          previousNoteEvent.absTimeSeconds + minNoteGapSeconds
      ) {
        skippedNotes.set(
          noteEvent.note,
          skippedNotes.get(noteEvent.note) ?? 0 + 1
        );
        skipCount++;
        currentNoteIndex++;
        continue;
      ...
      // Pagination
      if (
        noteEvent.absTimeSeconds + 0.5 * secondsPerHoleRadius >
        maxPageEndTime
      ) {
        ...
        if (previousNoteEvent) {
          ...
          const optimalPageEndTime =
            0.5 * (previousNoteEvent.absTimeSeconds + noteEvent.absTimeSeconds);
          const pageEndTime =
            optimalPageEndTime > maxPageEndTime
              ? maxPageEndTime
              : optimalPageEndTime;
    ...
    this.errors.push(
      `${skipCount} notes skipped (too close). Try increasing mm/sec. MIDI may contain overlapping notes.`
    );
  ...
  private filterNotes(
    supportedNoteSet: Set<MidiNote>,
    supportedEvents: NoteMidiEvent[],
    unsupportedEvents: NoteMidiEvent[]
  ) {
    const midiHeader = this.props.midiFile.getHeader();
    const midiTracks = this.props.midiFile.getTracks();
    const musicTrack: MidiTrack =
      midiHeader.fileFormat === MidiFileFormat.multiTrack
        ? midiTracks[1] // First track is tempor track for type 1 MIDI files
        : midiTracks[0];

    const noteOnEvents: NoteMidiEvent[] = musicTrack.events.filter(
      (e) =>
        e instanceof NoteMidiEvent &&
        (e as NoteMidiEvent).channelMessageType === ChannelMessageType.NoteOn
    ) as NoteMidiEvent[];
...
function createCircle(
  ...
  return (
    <circle
      key={key}
      cx={
        (midiEvent.absTimeSeconds - startTimeInSeconds) *
        musicBoxProfile.millimetersPerSecond
      }
      cy={noteOffsetY + noteIndex * noteGap}
      r={musicBoxProfile.holeDiameterMm / 2}
      fill={"none"}
      stroke={"black"}
      strokeWidth={0.25}
    />
